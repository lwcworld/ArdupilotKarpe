...
MAVLINK_HELPER void mavlink_quaternion_to_dcm(const float quaternion[4], float dcm[3][3])
{
	...
	dcm[0][1] = (float)(2.0 * (b * c - a * d));
	dcm[0][2] = (float)(2.0 * (a * c + b * d));
	dcm[1][0] = (float)(2.0 * (b * c + a * d));
	dcm[1][1] = (float)(aSq - bSq + cSq - dSq);
	dcm[1][2] = (float)(2.0 * (c * d - a * b));
	dcm[2][0] = (float)(2.0 * (b * d - a * c));
	dcm[2][1] = (float)(2.0 * (a * b + c * d));
...
MAVLINK_HELPER void mavlink_euler_to_quaternion(float roll, float pitch, float yaw, float quaternion[4])
{
    ...
    quaternion[0] = (float)((cosPhi_2 * cosTheta_2 * cosPsi_2 +
            sinPhi_2 * sinTheta_2 * sinPsi_2));
	quaternion[1] = (float)((sinPhi_2 * cosTheta_2 * cosPsi_2 -
            cosPhi_2 * sinTheta_2 * sinPsi_2));
	quaternion[2] = (float)((cosPhi_2 * sinTheta_2 * cosPsi_2 +
            sinPhi_2 * cosTheta_2 * sinPsi_2));
	quaternion[3] = (float)((cosPhi_2 * cosTheta_2 * sinPsi_2 -
            sinPhi_2 * sinTheta_2 * cosPsi_2));
}

MAVLINK_HELPER void mavlink_dcm_to_quaternion(const float dcm[3][3], float quaternion[4])
{
    quaternion[0] = (float)((0.5 * sqrt(1.0 +
            (double)(dcm[0][0] + dcm[1][1] + dcm[2][2]))));
	quaternion[1] = (float)((0.5 * sqrt(1.0 +
            (double)(dcm[0][0] - dcm[1][1] - dcm[2][2]))));
	quaternion[2] = (float)((0.5 * sqrt(1.0 +
            (double)(-dcm[0][0] + dcm[1][1] - dcm[2][2]))));
	quaternion[3] = (float)((0.5 * sqrt(1.0 +
            (double)(-dcm[0][0] - dcm[1][1] + dcm[2][2]))));
}
...
MAVLINK_HELPER void mavlink_euler_to_dcm(float roll, float pitch, float yaw, float dcm[3][3])
{
	...
	dcm[0][0] = (float)(cosThe * cosPsi);
	dcm[0][1] = (float)(-cosPhi * sinPsi + sinPhi * sinThe * cosPsi);
	dcm[0][2] = (float)(sinPhi * sinPsi + cosPhi * sinThe * cosPsi);

	dcm[1][0] = (float)(cosThe * sinPsi);
	dcm[1][1] = (float)(cosPhi * cosPsi + sinPhi * sinThe * sinPsi);
	dcm[1][2] = (float)(-sinPhi * cosPsi + cosPhi * sinThe * sinPsi);

	dcm[2][0] = (float)(-sinThe);
	dcm[2][1] = (float)(sinPhi * cosThe);
	dcm[2][2] = (float)(cosPhi * cosThe);
}
